...
def predict(filepath):
    ...
    if torch.any(outputs["instances"].pred_classes == 0):
        predicted = 'Benign'
    else:
        predicted ='Malignant'
...
def upload_file():
    ...
    if file:
        ...
        # Make prediction
        prediction_path,predicted = predict(file_path)

        with open(prediction_path, 'rb') as image_file:
            encoded_image = base64.b64encode(image_file.read()).decode('utf-8')

        # Return the result (including analyzed image)
        return jsonify({'analyzed_image': encoded_image,'prediction':predicted})
